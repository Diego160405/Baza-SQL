...
-- 1.upit - prikaz cijene svih proizvoda sa 10% popusta na svinjsko meso i 15% popusta na telece meso
...
SELECT 
    ...
    ROUND(
        CASE 
            WHEN vm.naziv = 'Svinjsko meso' THEN p.cijena_po_kg * 0.90
...
-- 2.upit - "Pronađi imena i prezimena zaposlenika koji su radili u smjeni između 08:00 i 16:00, i koji su zaprimili barem jednu narudžbu od kupca koji ima prezime 'Horvat'

CREATE VIEW zaposlenici_s_horvatom AS 
...
-- 3.upit - Prikazati popis proizvoda (vrste mesa i proizvodi), s njihovom cijenom i ukupnom količinom prodanom u svibnju 2025. (iz transakcija) no uzimamo samo podatke koji su ostavrili prodaju(ukupna kolicina prodaje je veca od 0kg), sortirano po količini prodaje od najveće do najmanje.
...
-- 4.upit - prikaz recenzije sa najvišom ocijenom i transakcije koje je obavio taj kupac sa imenom i prezimenom zaposlenika koji mu je prodao meso (želimo provjeriti sa kojom vrstom mesa i sa kojim zaposlenicama su kupci zadovoljni)
...
-- 5.upit - po broju narudžbi određujemo da li je narudžba složena ili jednostavna (jednostavne narudžbe se sastoje od 1 ili 2 proizvoda, a složene se sastoje od 3 ili više proizvoda)

CREATE VIEW jednostavna_ili_slozena AS
-- Jednostavne narudžbe (samo 1 proizvod)
SELECT 
    n.id AS narudzba_id,
    k.ime AS ime_kupca,
    k.prezime AS prezime_kupca,
...
-- 6.upit - prikaz svih proizvoda koji imaju nabavu zajedno sa onima koji je nemaju
