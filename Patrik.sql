...
-- 1. Popis svih proizvoda s vrstom mesa i cijenom po kilogramu
...
-- 2. Ukupna količina nabavljenog mesa po dobavljaču
SELECT dobavljac, SUM(kolicina) AS ukupna_kolicina
FROM nabava
GROUP BY dobavljac
ORDER BY ukupna_kolicina DESC;

-- 3. Najprodavaniji proizvod prema količini u narudžbama
...
-- 4. Ukupna zarada po proizvodu (na osnovu transakcija)
...
-- 7. Popis zaposlenika s njihovim smjenama
...
-- 8. Narudžbe i ukupna potrošnja po kupcu
...
-- 9. Proizvodi s cijenom većom od prosjeka svih proizvoda
SELECT naziv, cijena_po_kg
FROM proizvod
WHERE cijena_po_kg > (SELECT AVG(cijena_po_kg) FROM proizvod)
ORDER BY cijena_po_kg DESC;

-- 10. Provjera da li je mesnica trenutno otvorena (npr. za ponedjeljak u 14:00)
